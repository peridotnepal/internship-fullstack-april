  ...
  return (
    <div className="min-h-screen w-full p-4 md:p-8 bg-gradient-to-b from-zinc-950 to-zinc-950 text-gray-200  ">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">Dividend Information</h1>
          <p className="text-gray-400">View and search dividend data for various companies by time period</p>
        </div>

        <div className="bg-zinc-900 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-zinc-800">
          <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
            <div className="flex-1">
              <label className="block text-sm text-gray-400 mb-2">Select Time Period</label>
              <DropdownMenu>
            ...
            <>
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody className="bg-zinc-900">
                    {filteredData.map((stock, id) => (
                      ...
                      >
                        <td className="p-3 border-b border-zinc-800 text-gray-400">{id + 1}</td>
                        <td className="p-3 border-b border-zinc-800 font-medium text-blue-400">
                          {stock.Symbol}
                        </td>
                        <td className="p-3 border-b border-zinc-800">{stock.Company}</td>
                        <td className="p-3 border-b border-zinc-800 text-gray-300">
                          <span className="px-2 py-1 rounded-full text-xs bg-zinc-800">
                            {stock.Sector}
                          </span>
                        </td>
                        <td className="p-3 border-b border-zinc-800">{stock.LastTradedPrice}</td>
                        <td className={`p-3 border-b border-zinc-800 ${stock.BonusDividend > 0 ? 'text-green-400' : 'text-gray-400'}`}>
                          {stock.BonusDividend ? stock.BonusDividend.toFixed(2) : '0.00'}
                        </td>
                        <td className={`p-3 border-b border-zinc-800 ${stock.CashDividend > 0 ? 'text-green-400' : 'text-gray-400'}`}>
                          {stock.CashDividend ? stock.CashDividend.toFixed(2) : '0.00'}
                        </td>
                        <td className="p-3 border-b border-zinc-800 font-medium text-green-400">
                          {stock.TotalDividend ? stock.TotalDividend.toFixed(2) : '0.00'}
                        </td>
              ...
              {filteredData.length > 0 && (
                <div className="mt-4 text-sm text-gray-500 flex justify-between items-center px-2">
                  <span>Showing {filteredData.length} of {stockData.length} entries</span>
