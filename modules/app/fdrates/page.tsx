  ...
  return (
    <div className="max-w-7xl mx-auto p-6 md:p-10 space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-center gap-6 border-b pb-8">
        ...
        {/* View Toggle using Shadcn Buttons */}
      ...
      {viewMode === "table" ? (
          ...
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow>
                ...
                <TableHead className="text-center font-black">1 Year</TableHead>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentItems.map((rate) => (
            <Card key={rate.id} className="overflow-hidden border-2 ">
      ...
      <footer className="flex flex-col items-center gap-4 pt-4">
        ...
        <p className="text-xs text-muted-foreground italic">
          ...
          {Math.min(indexOfLastItem, fdRates.length)} of {fdRates.length} banks
        </p>
      </footer>
    </div>
  );
